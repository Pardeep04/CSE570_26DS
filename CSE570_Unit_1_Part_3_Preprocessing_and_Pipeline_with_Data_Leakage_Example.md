...
### Simple Real-Life Example of Data Leakage

Imagine a teacher wants to predict whether students will pass an exam.

- **Training data** = students used to teach the model.
- **Testing data** = new students used to check whether the model learned correctly.

The model should learn only from training students. If information from test students reaches the training process, it is like giving students the exam paper before the test.

#### Example: House Price Prediction

Suppose we want to predict house prices using the area of houses.
...
Then the same learned values are applied to the test data.
...
The test data is only transformed; it does not teach the model.

### Incorrect Approach (Data Leakage)

If scaling is performed before splitting:
...
Now the scaler has learned information from the test house also.

The model may show better performance, but the result is misleading because test data influenced training.

**Remember:**

> Training data teaches the model. Test data only checks the model.
