...
Suppose we include both city dummies:

$$
\widehat{\text{Price}}=b_0+b_1(\text{Delhi})+b_2(\text{Mumbai})
$$

If the fitted city prices are ₹50 lakh for Delhi and ₹70 lakh for Mumbai, all these coefficient choices give the same predictions:

| Intercept $b_0$ | Delhi coefficient $b_1$ | Mumbai coefficient $b_2$ | Delhi prediction | Mumbai prediction |
|---:|---:|---:|---:|---:|
| 0 | 50 | 70 | 0 + 50 = 50 | 0 + 70 = 70 |
| 10 | 40 | 60 | 10 + 40 = 50 | 10 + 60 = 70 |
| 30 | 20 | 40 | 30 + 20 = 50 | 30 + 40 = 70 |

**The data cannot tell us which coefficient set is the unique answer.** The predictions are the same, but the separate coefficients are not uniquely determined.

This happens because every row satisfies:

$$
D_{\text{Delhi}}+D_{\text{Mumbai}}=1
$$

The intercept already supplies a constant term. Together, the two dummy columns supply that same constant information again.

This exact redundancy is called the **dummy variable trap**, a form of **perfect multicollinearity**.

### 🟩 The solution: omit the reference category's dummy

Keep the intercept and the Mumbai dummy:

$$
\widehat{\text{Price}}=50+20(\text{Mumbai})
$$

- Delhi: $50+20(0)=₹50$ lakh.
- Mumbai: $50+20(1)=₹70$ lakh.

Both cities remain represented, and the coefficients have a clear meaning: **50 is the Delhi baseline, and 20 is Mumbai's difference from Delhi**.
