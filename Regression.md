...
## <span style="color:#7c3aed">🟪 7. Categorical inputs and dummy variables</span>

A **categorical variable** represents a group, such as city, department, or qualification. A regression equation needs a numerical representation of that group.

A **dummy variable** is an indicator that takes the value **1 if a category is present** and **0 otherwise**.

### 🟨 Example: New York and California

Use California as the **reference category**, also called the baseline category:
...
$$
\hat{y}=10+0.8R+3D_{\text{NY}}
$$

For this example, profit and R&D spending are measured in **thousands of dollars**.

At $R=20$:

- **California:** $10+0.8(20)+3(0)=26$, meaning predicted profit of USD 26,000.
- **New York:** $10+0.8(20)+3(1)=29$, meaning predicted profit of USD 29,000.

The coefficient 3 means that New York has predicted profit **USD 3,000 higher than California at the same R&D spending** in this model.

The reference category is still represented. It receives zero for the indicator and is captured by the intercept and other terms.

### Why not encode every category as 1, 2, 3?

For an unordered feature such as city, using Delhi = 1, Mumbai = 2, and Chennai = 3 as one numeric input imposes an artificial ordering and equal-sized steps. Separate indicators avoid that assumption.

Ordered categories, such as low, medium, and high, need deliberate treatment: assigning 1, 2, and 3 also assumes equal spacing if used directly in a linear term.

## <span style="color:#dc2626">🟥 8. The dummy variable trap</span>

Suppose we include an intercept and indicators for **both** New York and California:

$$
\hat{y}=b_0+b_1R+b_2D_{\text{NY}}+b_3D_{\text{CA}}
$$

If every observation belongs to exactly one of these two locations:

$$
D_{\text{NY}}+D_{\text{CA}}=1
$$

The intercept already contributes a constant column of ones. The two dummy columns add up to that same column, creating **perfect multicollinearity**, meaning an exact linear dependence among model columns.

### Why is this a problem?

The model cannot uniquely separate the intercept from both category effects.

For example, an intercept of 10 with category effects 3 and 0 produces the same predictions as an intercept of 11 with category effects 2 and −1. More than one coefficient set describes exactly the same predictions.

### The standard solution

> 🟩 **With an intercept and one categorical feature containing $k$ mutually exclusive categories, use $k-1$ dummy variables.** The omitted category becomes the reference group.

For three locations, choose Delhi as the baseline:

| Location | $D_{\text{Mumbai}}$ | $D_{\text{Chennai}}$ |
|---|---:|---:|
| Delhi | 0 | 0 |
| Mumbai | 1 | 0 |
| Chennai | 0 | 1 |

**Important qualification to the slides:** “Always omit one dummy variable” applies to the standard intercept-based setup. For a single categorical feature, a model without an intercept can use all $k$ indicators. With several categorical features, other dependencies may still arise. Always ensure that the full model has no exact redundant columns.

**Related issue:** even without exact dependence, strongly correlated inputs can make individual coefficients unstable. This is commonly called multicollinearity. Dropping one dummy addresses this particular exact dependency, not every possible source of multicollinearity.
