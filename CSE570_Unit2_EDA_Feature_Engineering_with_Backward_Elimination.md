...
**Backward elimination** is a **wrapper-based feature-selection method** that starts with all predictors and removes the least significant predictor one at a time.

**Steps:**
1. Select a significance level, usually `0.05`.
2. Fit the model using all predictors.
3. Identify the predictor with the highest p-value.
4. If the highest p-value is greater than the significance level, remove that predictor.
5. Refit the model with the remaining predictors and repeat the process.
6. Stop when all remaining predictors have p-values less than or equal to the significance level.

```python
# Backward elimination using p-values for the binary Churn target
import statsmodels.api as sm

X_be = df.select_dtypes(include="number").drop(columns=["Churn"]).copy()
X_be = X_be.fillna(X_be.median())
y = df["Churn"]
...
while True:
    # Fit logistic regression because Churn is a binary target
    model = sm.Logit(y, X_be).fit(disp=False)

    # Ignore the intercept and find the predictor with the highest p-value
    p_values = model.pvalues.drop("const")
    highest_p_value = p_values.max()

    # Remove the least significant predictor if its p-value is above 0.05
...
selected_backward = X_be.drop(columns=["const"]).columns
print("Selected features using backward elimination:", list(selected_backward))
```

### Backward Elimination Flowchart

![Backward Elimination Flowchart](Backward_Elimination_Flowchart.png)
...
Feature transformation does **not remove a feature**. It changes the way the feature is represented.

- **Scaling** → brings numerical features to a similar range, so large-valued features do not dominate only because of their scale.
- **Log transformation** → reduces the effect of very large values and can make highly skewed data easier to model.
- **Categorical encoding** → converts text categories into numerical form so machine-learning algorithms can use them.
...
Original data
     ↓
Age = 25, MonthlyCharges = 5000, ContractType = "Yearly"
     ↓
Feature Transformation
     ↓
Scaled numbers + encoded categories
```

> **Easy idea:** Feature transformation changes the **form of data**, not the meaning of the feature.
